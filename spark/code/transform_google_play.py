...
def solution(classification):
    ...
    for i in range(len(list_url)):
        ...
        data.append(tmp)
	# spark-submit /opt/airflow/code/transform_google_play.py
    df_ = spark.createDataFrame(data)
    # df_.write.mode("overwrite").csv("hdfs://namenode:9000/" + classification + "/" + "test612")
...
if __name__ == "__main__":
	spark = SparkSession.builder.master("spark://spark-master:7077").config("spark.jars", "/opt/airflow/code/postgresql-42.2.5.jar").appName("transform_google_play").getOrCreate()
	# spark = SparkSession.builder.appName("write_to_htfs").getOrCreate()
